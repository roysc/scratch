...
struct _indices_of<Variadic<Ts...> >
{ using type = _build_indices<sizeof...(Ts)>; };
}

template <class Variadic>
using indices_of = typename
  detail::_indices_of<typename std::remove_reference<Variadic>::type>::type;
...
{ _expand_apply(f, tuple, indices_of<Variadic> {}); }
